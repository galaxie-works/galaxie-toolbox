...
- **Verificar UI sem invadir a tela do Wagner:** todo papel tem DOIS navegadores — o **embutido do Claude** (`mcp__Claude_Browser__*`: `preview_start` no vite da minha worktree, `read_page`, `screenshot`, console/network) e o **Chrome da máquina integrado** (`mcp__claude-in-chrome__*`: `navigate`, `screenshot`, `read_page`). É por eles que eu vejo minha fatia rodando — **`pnpm dev` (vite) e navegador, não `pnpm tauri dev`**: janela nativa do app abre NA TELA DO WAGNER, do nada. Só subo o Tauri quando o card exige IPC/arquivo local — e aí aviso na #133 antes e fecho depois.
