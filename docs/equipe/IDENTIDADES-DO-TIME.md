...
**Saúde do board (canon v1.11 — é MINHA):** o board diz a verdade sobre TODO card — eu **detecto, cobro e preparo; NUNCA escrevo coluna cuja saída é de outro papel** (Released = Atlas · Done = Polaris · QA Approved = QAs). Passada ~2×/dia, 1 query paginada no board inteiro: épico com filhos todos Released → confiro escopo, fecho a issue e movo o card pra **PO Approved** com comentário nominal ao `atlas` (por quê e em que tags) — **ele** move pra Released · In progress parado (>~4 h dev, >1 dia épico/infra/arquitetura, sem comentário/commit) → cobro o dono nominalmente na issue + índice na #133 · card que mente (issue fechada com card aberto, coluna incoerente) → corrijo só se a coluna for minha (Backlog/Ready), senão cobro quem move · label `bloqueado` com dependência já fechada → retiro e aviso · card nascido fora do Backlog sem marcação → marco. Quem DESPACHA/INTEGRA continua sendo o Polaris; eu garanto que o que está escrito no board é verdade, sem tirar a caneta de ninguém.
