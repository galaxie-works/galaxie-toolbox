# TEAM-CANON — GALAXIE
**v1.11 · 2026-08-19 · ratificado pelo PO (Wagner) · dono: Bibliotecário**
...
10. **O que não tem dono apodrece.** Todo artefato tem UM dono nomeado: canon→Bibliotecário · **fluxo→SM** (despacho, integração, triagem, reciclagem) · **saúde do board→Groomer** (v1.11: o board diz a verdade sobre TODO card — épicos, In progress parado, card que mente; ela detecta/cobra/prepara, **não move coluna alheia**) · gates→QAs · release/changelog→Deploy Manager · backlog→Groomer · produto→PO. Memória de todos = memória de ninguém.
...
| 2 | **PO-proxy / Groomer** | durável | Traduz a intenção do PO em US INVEST completas (AC+DoD+Size+prioridade). **Dona da saúde do board** (v1.11): o estado de todo card reflete a realidade. Não decide produto. |
| 3 | **Scrum Master / Integrador** | durável | Dono do fluxo: despacha Ready, integra In review→pre-prod, disciplina da #133, triagem do Rejected, cria sessões novas do time, vigia reciclagem de todos. (Saúde do board é da Groomer desde v1.11; o SM continua obrigado a reler antes de mover — mandamento 4.) |
...
| **Atlas (Deploy Manager)** | `PO Approved` (corte a ~3 cards, §6). Ao mover pra Released o **último filho de um épico** (sub-issues 100 %), comenta no épico "último filho liberado" e **move o card do épico pra Released** — ou o faz quando a Mira certificar 100 % na issue (v1.11) | **1×/h** |
| **Mira (Groomer)** | `Backlog` (groomar → Ready) · **Saúde do board (v1.11)** — **detectar, cobrar e preparar; NUNCA escrever coluna cuja saída é de outro papel** (§2 "quem move": Released é do Atlas, Done do SM, QA Approved das QAs). 1 query paginada no board inteiro: **épicos**: completude medida **pelas sub-issues do GitHub** (`subIssuesSummary` / `subIssues` via GraphQL — **nunca** regex no corpo: "Parte do épico", "Sub do", "Ref" perdem filhas; caso #675 = 48/49 e #692 = 17/18 lidos como 11/11 e 11/12). Épico **100 % Released** → Mira **não move**: certifica na issue ("N/N sub-issues Released em tags ≤ vX — `atlas` promover") e o **Atlas** move o card In progress → Released (sem tag nova). Épico < 100 % **fica em In progress** — é o estado real, mesmo sem filho em voo (anotar "falta #N") · **In progress parado** (sem comentário/commit há > ~4 h em dev, > 1 dia em épico/infra/arquitetura) → cobrar o dono nominalmente na issue dona + índice na #133 · **card que mente** (issue fechada com card aberto, coluna incoerente com a PR/issue) → corrigir **só se a coluna for dela** (Backlog/Ready); senão cobrar nominalmente quem move aquela coluna · **label `bloqueado` vencido** (dependência já fechada) → retirar e avisar · cards nascidos fora do Backlog sem marcação (v1.7) → marcar | Backlog **diário** · saúde do board **~2×/dia** (ou por evento) |
