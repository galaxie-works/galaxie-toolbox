...
10. **O que não tem dono apodrece.** Todo artefato tem UM dono nomeado: canon→Bibliotecário · **fluxo→SM** (despacho, integração, triagem, reciclagem) · **saúde do board→Groomer** (v1.11: o board diz a verdade sobre TODO card — épicos, In progress parado, card que mente) · gates→QAs · release/changelog→Deploy Manager · backlog→Groomer · produto→PO. Memória de todos = memória de ninguém.
...
| **Mira (Groomer)** | `Backlog` (groomar → Ready) · **Saúde do board (v1.11)** — 1 query paginada no board inteiro: **épicos** cujos filhos estão todos Released → conferir escopo, fechar, mover pra Released · **In progress parado** (sem comentário/commit há > ~4 h em dev, > 1 dia em épico/infra/arquitetura) → cobrar o dono nominalmente na issue dona + índice na #133 · **card que mente** (issue fechada com card aberto, coluna incoerente com a PR/issue) → corrigir ou cobrar quem move · **label `bloqueado` vencido** (dependência já fechada) → retirar e avisar · cards nascidos fora do Backlog sem marcação (v1.7) | Backlog **diário** · saúde do board **~2×/dia** (ou por evento) |
