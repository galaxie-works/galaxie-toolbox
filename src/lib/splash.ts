 ...
 *
 * A janela `splashscreen` (círculo transparente) abre visível já no launch; a
 * janela `main` nasce oculta (`visible:false`) e só aparece quando o boot da
 * `App` termina. Este módulo é importado SÓ pela árvore da main window (o
 * `main.tsx` desvia a splash para o `SplashScreen`), então `getCurrentWindow()`
 * aqui é sempre a `main`.
 *
