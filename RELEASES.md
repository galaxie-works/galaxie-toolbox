...
| Versão | Resumo |
|---|---|
| **v0.60.0** | ⚙️ Salvar preferências pela plataforma (`PATCH /me/config` por chave, forma server-side); org não fica órfã (último OrgAdmin não sai/rebaixa, 409 legível) + remover membro; auditoria diz contra quem (enum fechado) e anti-injeção provada em produção; rótulos "The GALAXIE"; splitter do Files = Bridge; CI publica remote-signaling no GHCR. |
