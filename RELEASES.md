...
| Versão | Resumo |
|---|---|
| **v0.62.0** | ⚙️ Isolamento de prefs PROVADO (guard real do definir_pref, mutante morde); detetor de Definição-de-Ready no board.ps1; Remote avisa expiração da credencial TURN (fatia C); turn_relay_echo (peer da travessia #1130-b); roster JSON com guarda tudo-ou-nada; galaxie.db (SQLite de histórico); cofre DPAPI dos client_secret OAuth; canon v1.20–v1.21. |
| **v0.61.0** | 🛡️ Guarda ponto-único fechada por todos os eixos (Alerta ponto-único + contraste AA, style inline/spread, alias de import); GitHub Actions pinadas por SHA (supply-chain, 0 tag mutável); runbook de DR máquina-nova (cofre re-registra, memory restaura); prova TURN: credencial aceita pelo nosso realm (E2E → #1130-b); CI de imagem valida PR sem packages:write; canon v1.17–v1.19 (conta-por-papel, gate de threads). |
| **v0.60.0** | ⚙️ Caminho de escrita de preferências completo na borda+crate (`PATCH /me/config`, forma server-side; produção aguarda armazém durável — fatia seguinte); org não fica órfã (último OrgAdmin não sai/rebaixa, 409 legível) + remover membro; auditoria diz contra quem (enum fechado) e anti-injeção provada no caminho real; rótulos "The GALAXIE"; splitter do Files = Bridge; CI publica remote-signaling no GHCR. |
| **v0.59.0** | 🕵️ Trilha de auditoria da plataforma: ações de admin da org auditadas (simetria back-office), auditoria por construção (autorizar sem auditor não compila), sondagem de config alheia registrada; `GET /me/config` na borda; org suspensa sem gaguejo; Remote: 1º comando do pipe SYSTEM (Ping) + contrato Register v2 documentado. |
